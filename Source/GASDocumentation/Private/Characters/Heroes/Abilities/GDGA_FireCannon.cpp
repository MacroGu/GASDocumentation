...
#include "Characters/Heroes/Abilities/GDGA_FireCannon.h"
...
UGDGA_FireCannon::UGDGA_FireCannon()
{
...
void UGDGA_FireCannon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	Task->OnBlendOut.AddDynamic(this, &UGDGA_FireCannon::OnCompleted);
	Task->OnCompleted.AddDynamic(this, &UGDGA_FireCannon::OnCompleted);
	Task->OnInterrupted.AddDynamic(this, &UGDGA_FireCannon::OnCancelled);
	Task->OnCancelled.AddDynamic(this, &UGDGA_FireCannon::OnCancelled);
	Task->EventReceived.AddDynamic(this, &UGDGA_FireCannon::EventReceived);
	// ReadyForActivation() is how you activate the AbilityTask in C++. Blueprint has magic from K2Node_LatentGameplayTaskCall that will automatically call ReadyForActivation().
	Task->ReadyForActivation();
}

void UGDGA_FireCannon::OnCancelled(FGameplayTag EventTag, FGameplayEventData EventData)
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, true);
}

void UGDGA_FireCannon::OnCompleted(FGameplayTag EventTag, FGameplayEventData EventData)
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UGDGA_FireCannon::EventReceived(FGameplayTag EventTag, FGameplayEventData EventData)
{
